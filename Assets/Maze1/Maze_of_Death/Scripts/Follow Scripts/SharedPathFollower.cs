...
public class SharedPathFollower : MonoBehaviour, IGetBlastTank
{
    ...
    private void Awake()
    {
        ...
        defaultSpeed = 0.5f; // same in both cases
    }
    ...
    private IEnumerator UpdatePathRoutine()
    {
        while (true)
        {
            ...
            else if (playerDetected)
            {
                ...
                speed = maxSpeed;
                pathUpdateInterval = 0.15f;
                SetEnemyCollisionState(false); // disable enemy collisions
            }
            else
            {
                isChasingPlayer = false;
                ...
                speed = defaultSpeed;
                pathUpdateInterval = 0.25f;
                SetEnemyCollisionState(true); // restore collisions
            }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.CompareTag("Player"))
        {
            ...
            attackCoroutine = StartCoroutine(RotateThenAttack(targetRot));
    ...
    {
        if (playerDetected && collision.collider.CompareTag("enemy"))
        {
            SharedPathFollower otherEnemy = collision.collider.GetComponent<SharedPathFollower>();
            if (otherEnemy != null)
            {
                // Direction to push away
                Vector2 pushDir = (transform.position - otherEnemy.transform.position).normalized;

                // Small push to separate enemies
                rb.MovePosition(rb.position + pushDir * 0.05f);
            }
        }
    }
    ...
    // ---------------- NEW: COLLISION STATE ----------------
    private void SetEnemyCollisionState(bool allowCollide)
    {
        if (allowCollide)
            gameObject.layer = LayerMask.NameToLayer("Enemy");
        else
            gameObject.layer = LayerMask.NameToLayer("EnemyNoCollide");
    }
}*/
